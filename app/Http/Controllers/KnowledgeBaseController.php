...
class KnowledgeBaseController extends Controller
{
        ...
        if ($request->ajax()){
            ...
            return DataTables::of($data)
                ->addColumn('content', function ($data){
                    $tp = '<a href="#" style="white-space: nowrap;" data-toggle="tooltip" title="'.strip_tags($data->content).'" role="tooltip" data-original-title="Default tooltip">'
                            .strip_tags(str_limit($data->content, 40)).
                            '</a>';
                ...
                ->addColumn('action', function ($data){
                    ...
                    if(isset($data->file)){
                         ...
                         $filePath = route('showpdf', pathinfo($data->file, PATHINFO_FILENAME));
                        $value .= '<a href="'.$filePath.'" class="btn btn-warning btn-sm" target="_blank" title="file"><i class="fa fa-paperclip"></i></a>';
                    }
                    

                    return $value;
                })
                ->rawColumns(['content','pinned_status','kb_status','action'])->make(true);
        }

        return view('kb.index', compact('departments'));
    }
        public function knowledgesIndex(){
                  $catgegories = Department::all();
                  $posts = KnowledgeBase::latest()->where('status',KnowledgeBase::PUBLISHED)->limit('10')->get();
                  return view('kb.cards', compact('catgegories','posts'));
        }
    ...
    public function showpdf($pathToFile){
       
         $path = 'public/files/uploads/'.$pathToFile.'.pdf';
        $headers = [
    ...
    public function store(Request $request)
    {
        ...
        if($request->file('file'))
        {
            ...
            $img=  $file->move($filePath, $filename);
            DB::table('knowledge_bases')->where('id', $created->id)->update(['file' => $filename]);
        }
