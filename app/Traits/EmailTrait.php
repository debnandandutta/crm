...
trait EmailTrait
{
    ...
    protected function newTicketSubmitTemplate($user,$ticketId, $priority, $status)
    {
        $type = 'ticket_info';
        $text = $this->getEmailTemplate($type);
        $ticketLink = \Request::root().'/enquiry/'.$ticketId;
        $template = str_replace('{app_name}', $this->appSetting()->app_name,str_replace('{user_name}', $user->name, str_replace('{ticket_priority}', ucfirst($priority), str_replace('{ticket_status}',$status,str_replace('{ticket_view_url}',$ticketLink, $text)))));
