...
@section('main-section')
    <div class="container-fluid">
        @if($permission->manageStaff() == 1)
            <h4 class="page-title">{{ __('lang.edit_town') }}
            ...
            <div class="row">
                <div class="col-md-12">
                    ...
                    <!-- /.box-header -->
                        <!-- form start -->
                        <form role="form" class="" method="POST" action="{{ route('update-town') }}">
                            {!! csrf_field() !!}
                            ...
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <input id="name" type="text" class="form-control {{ $errors->has('name') ? ' is-invalid' : '' }}" name="name" value="{{ $town->townName }}" placeholder="{{ __('lang.enter_name') }}" required>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select id="region" class="form-control" name="region" required>
                                                ...
                                                @foreach ($regions as $region)
                                                    <option value="{{ $region->id }}" {{ $town->regionId === $region->id ? 'selected' : '' }}>{{ $region->title }}</option>
                                                @endforeach
                                            </select>
                                ...
                                <div class="box-footer mb-3">
                                    <button type="submit" class="btn btn-primary ">{{ __('lang.edit_town') }}</button>
                                </div>
                            </div>
                            <!-- /.box-body -->
                        </form>
